...
[refs](/api/reactivity-core.html#ref) returned from `setup` are [automatiquement désenveloppés à la surface](/guide/essentials/reactivity-fundamentals.html#deep-reactivity) when accessed in the template so you do not need to use `.value` when accessing them. They are also unwrapped in the same way when accessed on `this`.
