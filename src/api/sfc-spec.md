...
- A `<style>` tag can have `scoped` or `module` attributes (see [les fonctionnalités de style pour les composants monofichiers](/api/sfc-css-features) for more details) to help encapsulate the styles to the current component. Multiple `<style>` tags with different encapsulation modes can be mixed in the same component.
