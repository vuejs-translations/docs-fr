...
La fonction `computed()` est généralement utilisée dans les composants utilisant l'API Composition, mais peut également être utilisée pour compléter certains cas d'usage avec l'API Options. Vous pouvez en savoir plus sur son utilisation en lisant [les principes de base de la réactivité](/guide/essentials/reactivity-fundamentals.html) et [les propriétés calculées](/guide/essentials/computed.html) avec la préférence d'API définie sur API Composition.
