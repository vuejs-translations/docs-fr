...
Les types de saisie intégrés dans HTML ne répondent pas toujours à vos besoins. Heureusement, les composants Vue vous permettent de construire des entrées réutilisables avec un comportement complètement personnalisé. Ces entrées fonctionnent même avec `v-model` ! Pour en savoir plus, lisez l'article [Utilisation avec `v-model`](/guide/components/v-model.html) dans le guide des composants.
