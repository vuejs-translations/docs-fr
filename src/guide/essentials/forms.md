...
HTML's built-in input types won't always meet your needs. Fortunately, Vue components allow you to build reusable inputs with completely customized behavior. These inputs even work with `v-model`! To learn more, read about [utilisation avec `v-model`](/guide/components/v-model.html) in the Components guide.
